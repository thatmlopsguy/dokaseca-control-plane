Copyright (c) DoKa Seca contributors

This product includes software developed by contributors to the DoKa Seca project.

DoKa Seca is licensed under the Apache License, Version 2.0.
See the LICENSE file for the full license text.

Documentation license:

The documentation in this repository (all files under `docs/`, unless noted otherwise)
is licensed under the Creative Commons Attribution 4.0 International License (CC BY 4.0).

You can read the full license text here:
<https://creativecommons.org/licenses/by/4.0/>

SPDX identifier: `CC-BY-4.0`

Third-party components:

This project may include third-party components, which are licensed under their respective licenses.
Please refer to the NOTICE file for details on third-party components and their licenses.
